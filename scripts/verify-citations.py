...
URL_IN = re.compile(
    r"(?:https?://[^\s`,)\]]+|`(?:https?://)?[a-z0-9.-]+\.[a-z]{2,}/[^`\s]*`)"
)
ZDROJ_START = re.compile(r"^\s*\*\*ZDROJ")
FRAGMENT_SPLIT = re.compile(r"\(\s*\.\.\.\s*\)|\.\.\.|…")
...
# Bez "a" a "to": obě slova jsou zároveň česká i anglická a rozhodovala by špatně.
EN_STOPWORDS = set(
    "the an of is are and or in for be that this with when should not can it as by "
    "you your on at from use must will has have".split()
)
# Jen slova, která se v anglickém textu nevyskytnou ani po rozpadu na [a-z]+: "ten", "co",
# "si", "v", "k" nebo "pro" by z anglické citace udělaly českou (ten, co-founder, SI, v2, 4k).
CZ_STOPWORDS = set(
    "ve na se je ne nebo ale jak od po za bez jen aby jsou ma musi nesmi bude byt kdyz "
    "tak ani nic".split()
)
...
def is_czech(s: str) -> bool:
    ...
    Diakritika sama nestačí: „ne spinner v tabulce" ji nemá, proto se ještě hledají funkční
    slova. Bez důkazu na obě strany se citace považuje za cizí a ověří se: falešný poplach
    CITACE PRYC je u brány lepší selhání než tichý průchod bez ověření.
    """
    if sum(1 for c in s if c in CZ_DIACRITICS) / max(len(s), 1) > 0.01:
        return True
    words = set(re.findall(r"[a-z]+", s.lower()))
    return bool(words & CZ_STOPWORDS) and not (words & EN_STOPWORDS)
...
def fragments(quote: str) -> list[str]:
    ...
    Takovou citaci nelze hledat jako jeden řetězec, ale každý úsek zvlášť ano. Úseky pod
    patnáct znaků se vynechávají, protože sedí náhodně; když je krátký každý, hledají se
    všechny, nikdy původní řetězec i s výpustkou.
    """
    parts = [p for p in (x.strip() for x in FRAGMENT_SPLIT.split(quote)) if p]
    return [p for p in parts if len(p) >= 15] or parts
...
def _pdf_text(url: str) -> str:
    ...
    try:
        got = subprocess.run(
            ["curl", "-sSL", "--max-time", "120", "-o", raw, url],
            capture_output=True,
            timeout=180,
        )
...
def _run(url: str, browser: bool) -> tuple[str, str]:
    ...
    cmd = (
        ...
        if browser
        else ["scrapling", "extract", "get", url, out]
    )
    try:
        proc = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
        if proc.returncode != 0:
            # částečný výstup po pádu by se jinak porovnal jako celá stránka
            return "", url
        m = re.search(r"<GET (\S+)>", proc.stderr or "")
        return Path(out).read_text(encoding="utf-8", errors="replace"), (
            m.group(1) if m else url
        )
    except (OSError, subprocess.SubprocessError):
        return "", url
    finally:
...
def fetch(url: str, browser: bool = False) -> tuple[str, str]:
    ...
    if url.lower().split("?")[0].endswith(".pdf"):
        return _pdf_text(url), url
    text, final = _run(url, browser)
    if not browser and len(text) < THIN:
        # přes fetch, ne _run, ať se prohlížečový výsledek zacachuje i pod (url, True)
        text2, final2 = fetch(url, True)
    ...
    while i < len(lines):
        if ZDROJ_START.match(lines[i]):
            ...
            while (
                i < len(lines)
                and lines[i].strip()
                and not (buf and ZDROJ_START.match(lines[i]))
            ):
                buf.append(lines[i])
                i += 1
...
def check(path: Path) -> tuple[int, int]:
    ...
    for lineno, text in units(path.read_text(encoding="utf-8").splitlines()):
        ...
        if not urls:
            continue
        quotes = []
        for q in CZ_QUOTE.findall(text):
            if is_czech(q):
                # vyřazení musí být vidět, tichý skip by bránu obešel bez stopy
                print(f"  PRESKOCENO  {path.name}:{lineno}  {q[:54]}...")
            else:
                quotes.append(q)
...
if __name__ == "__main__":
    ...
    if not shutil.which("scrapling"):
        # bez teto kontroly by chybejici nastroj vypadal jako NEDOSTUPNE u kazde URL
        print("CHYBA: `scrapling` neni na PATH, viz docstring skriptu", file=sys.stderr)
        raise SystemExit(2)
    tot_c = tot_p = 0
