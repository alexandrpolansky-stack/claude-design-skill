...
| [enterprise-ui/](enterprise-ui/) | 32 | Produktové aplikace: volba komponenty podle úkolu, vzory (prázdné stavy, notifikace, filtrování), komponenty (tabulky, formulářové prvky, taby), klávesnice a čtečky. Principy z IBM Carbonu, bez jeho vizuálu; několik not stojí na jiných zdrojích a každá to u sebe říká. |
...
- [Souhrny a agregace](enterprise-ui/vzory/souhrny-a-agregace.md) - **pruh čísel nad daty, který tvrdí klid, zatímco jeden člen hoří.** Proč agregát přes nerovnoměrně vytížené jednotky zamlčí toho, kdo je v problému, a jak se pozná, kdy součet na otázku o zásahu vůbec neodpovídá. Dál definice dashboardu jako funkční požadavek, přehled rozřezaný na výběry, číslo bez kontextu, **odchylka v procentech u různě velkých jednotek**, prominence podle toho, co potřebuje pozornost teď, rozmanitost zobrazení jako anti-vzor a proč nepoužitý signál patří pryč. Zdroje mimo Carbon: Stephen Few a Google SRE.
